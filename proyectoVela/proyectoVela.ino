...
void loop() {
  ...
  if((millis() - delayInicial) >= 150){//Cada 200ms tomamos una medida
    delayInicial = millis();
    
    if((medida[1] == 0) && (medida[0] != 0)){//Si tenemos una sola medida...
       medida[1] = tensionNTC;

        if((medida[0]-medida[1]) > 5)//Si la pendiente decrece no es por un pulso espurio...
            digitalWrite(rele, HIGH);
          else if((medida[1] < 150) && ((medida[0]-medida[1]) > 1))//Si no hay pendiente pero se esta aplicando fuego...
           digitalWrite(rele, HIGH);
